					...
					<div class="hdc">
						<div class="hdi">
							<div class="login">
								...
								<div class="shopcart">
									<em class="text">
									<p class="fr">购物车(<b>{{if value="isset($_SESSION['cart'])"}}
											{{count($_SESSION['cart'])}}
										{{else}}
											0
										{{endif}}</b>)</p>
		...
		<div id="main">
			<div class="myls-left">
				...
				<div class="myls-lc">
					...
					<div class="myls-lct">
						<form method="post" id="form" action="{{U('Home/Member/my_orders')}}">
							<div class="subsort">
								<a href="" class="select">未付款({{$error}})</a>
								<span>|</span>
								<a href="{{U('Home/Member/my_payed')}}">已付款({{$success}})</a>
								<span>|</span>
								<!-- <a href="">已取消</a> -->
										...
										<tr>
											<td class="left">
												<div class="order-info">
													<!-- <input name="trade_no[]" value="{{$k}}" checked="" type="checkbox"> -->
													<a class="orders-img" href="#">
														<img src="{{$v['pic']}}" height="54" width="85">
													</a>
													<p>
														<a href="#" title="{{$v['gname']}}">{{$v['gname']}}</a>
														<br>订单号：{{$v['number']}}
													</p>
												</div>
											</td>
											<td>{{$v['quantity']}}</td>
											<td>¥{{$v['subtotal']}}</td>
											<input type="hidden" name="account[]" value="{{$v['subtotal']}}" />
											<td>
												<span class="clr01">{{$v['status']}}</span>
											</td>
											<td>
												<p>
													<a class="order-but" href="{{U('Home/Member/oneGoodPay',array('oid'=>$v['oid'],'account'=>$v['subtotal']))}}">支付</a>
												</p>
												<p>
			...
			<div class="myls-right">
				<div class="myls-rwarp myls-rwarptop">
					...
					<div class="myls-account">
						...
						<p><a href="">账户余额</a>：<span>¥&nbsp;{{$account}}</span></p>
