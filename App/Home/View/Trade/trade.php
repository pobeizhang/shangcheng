	...
	<body>
		...
		<div class="narrow-main">
			<div class="narrow-content">
				...
				<form action="{{U('Home/Trade/zhifu')}}" method="post" enctype="application/x-www-form-urlencoded" name="postpay" id="payForm" target="_blank">
					<dl class="buy-details">
						<dt>
							<h3>
								商品信息
							</h3>
							<span>应付总额：</span><em>¥</em><strong>{{$orders_prices}}</strong>
						</dt>
						...
						<dd class="cl">
							<p class="left" style="display: inline-block;width: 70px;">
								<img src="{{$v['pic']}}" alt="" style="width:70px;"/>
							</p>
							<p class="left">
							...
							<p class="left">
								<span>单件价格{{$v['subtotal']/$v['quantity']}}</span>
								<span>{{$v['quantity']}}件</span>
					...
					<div class="money-details">
						<p>
							<span>需要支付：</span><em>¥</em><strong>{{$orders_prices}}</strong>
							<input type="hidden" name="account" value="{{$orders_prices}}" />
						</p>
						<p>
							<span>你的账户余额：</span><em>¥</em><strong>{{$account}}</strong>
						</p>
					</div>
					<div class="buy-botton">
						<!-- <input name="trade_no" id="trade_no" value="1162648488w4c7ae7957" type="hidden"> -->
						<a href="{{U('Home/Index/index')}}" >暂不支付，继续购物</a>
